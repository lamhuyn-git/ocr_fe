...
function MethodCard({ icon, title, subtitle, onClick }: MethodCardProps) {
  return (
    ...
    >
      <img src={icon} alt="" className="w-12 h-12 rounded-xl shrink-0 object-cover" />
      <div className="flex flex-col gap-1">
        <span className="text-sm font-bold text-text-main leading-none">{title}</span>
        <span className="text-sm font-normal text-text-main leading-[1.45]">{subtitle}</span>
      </div>
    </button>
  );
}
...
export default function LoginMethodSelect({ onSelect }: LoginMethodSelectProps) {
  return (
